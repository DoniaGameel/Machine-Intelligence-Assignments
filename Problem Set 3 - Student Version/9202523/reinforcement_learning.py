...
class RLAgent(Agent[S, A]):
    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        ...
        if training and self.should_explore():
            ...
            # Get the list of actions with the maximum q-value
            best_actions = [action for action, q_value in zip(actions, q_values) if q_value == max_q_value]
            # Return the first action in the list (if there are ties)
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        # Compute the utility of the state as the maximum Q-value over all possible actions
        actions = env.actions()
        if not actions:
            return 0.0  # No available actions, utility is 0

        max_q_value = max(self.compute_q(env, state, action) for action in actions)
        return max_q_value
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        # Weighted sum of features for the given action
    ...
    def __compute_utility_from_features(self, features: Dict[str, float]) -> float:
        # TODO: Complete this function
        # Utility is the maximum Q-value over all actions
        max_q_value = max(self.__compute_q_from_features(features, action) for action in self.actions)
        return max_q_value
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        # If done is True, then next_state is a terminal state in which case, we consider the Q-value of next_state to be 0
        # Extract features for the current state and the next state
        current_features = self.feature_extractor.extract_features(env, state)
        next_features = self.feature_extractor.extract_features(env, next_state)

        # If done is True, set the Q-value of the next state to 0
        if done:
            next_q_value = 0
        else:
            # Compute the maximum Q-value over all actions for the next state
            next_q_value = self.__compute_utility_from_features(next_features)
        
        # Thus the approximate Q-function can be written as follows:
        approximate_Q_value = self.__compute_q_from_features(current_features, action)
        # Compute the target Q-value using the Q-Learning update rule
        target_q_value = reward + self.discount_factor * next_q_value -  approximate_Q_value

        # Update the weights for the chosen action
        for feature in current_features:
            self.weights[action][feature] += self.learning_rate * (target_q_value * current_features[feature])
