...
class CryptArithmeticProblem(Problem):
    ...
    def from_text(text: str) -> 'CryptArithmeticProblem':
        ...
        problem.domains[LHS0[0]].difference_update({0})  # First letter cannot be 0
        problem.domains[LHS1[0]].difference_update({0})  # First letter cannot be 0
        problem.domains[RHS[0]].difference_update({0})  # First letter cannot be 0
        problem.constraints = []
        ...
        '''
        # Extract all letters and carries
        letters = set(LHS0 + LHS1 + RHS)
        carries = set(chr(ord('A') + i) for i in range(len(letters) - 1))

        # Add variables to the problem
        problem.variables = list(letters) + list(carries)

        # Set domains for letters (0-9) and carries (0-1)
        problem.domains = {letter: set(range(10)) for letter in letters}
        problem.domains.update({carry: {0, 1} for carry in carries})
        problem.domains[LHS0[0]].difference_update({0})  # First letter cannot be 0
        problem.domains[LHS1[0]].difference_update({0})  # First letter cannot be 0
        problem.domains[RHS[0]].difference_update({0})  # First letter cannot be 0
        problem.constraints = []
        # Add constraints the letter takes a value from its domain 
        for letter in letters:
            problem.constraints.append(UnaryConstraint(letter, lambda value: value in problem.domains[letter]))

        # Add constraints for the letter to take only values from its domain
        for letter in letters:
            problem.constraints.append(UnaryConstraint(letter, lambda value: value in problem.domains[letter]))
        # Add constraint for all letters to be different
        letters = list(letters)
        for i in range(len(letters)):
            for j in range(i + 1, len(letters)):
                variable1, variable2 = letters[i], letters[j]
                problem.constraints.append(
                    BinaryConstraint((variable1, variable2), lambda x, y: x != y)
        )
